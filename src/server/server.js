...
const db = mysql.createConnection({
    host : 'localhost',
    user: 'my-react',  //생성
    password: 'react-board-pw',
    port: 3306,
    database:'my_react_board' //생성
});
...
// get full constellation list
app.get('/', (req, res) => {
    res.send("React Server Connect Successed.");
});

app.get('/list', (req, res) => {
    console.log('data sending to "/list" path.');
    const sql = `SELECT * FROM conste order by conste_id desc`;
    db.query(sql, (err, data) => {
        if(err) {
            res.send('Failed to get data');
            console.log('Getting all from conste failed: ', err);
        } else {
            res.send(data);
        }
    })
...
app.get('/users', (req, res) => {
    console.log('data sending to "/users" path.');
    const sql = `SELECT * FROM userInfo order by user_num`;
    db.query(sql, (err, data) => {
        if(err) {
            res.send('Failed to get data');
            console.log('Getting all from userInfo failed: ', err);
        } else {
            res.send(data);
        }
    })
...
app.post('/addUser', (req, res)=>{
    ...
    const sql = `INSERT INTO userInfo(user_id, user_pw, user_name, user_birth) VALUES (?, ?, ?, ?) `;
    db.query(sql, [userId, userPw, userName, userBirth], (err, data)=>{
        if(err){
            console.log('Adding userInfo error: ', err);
        }else{
            res.sendStatus(200);
        }
    })
...
app.get('/findUser', (req, res) => {
    console.log('Start user finding...');
    const sql = `SELECT user_id, user_pw FROM userInfo ORDER BY user_num`;
    db.query(sql, (err, data) => {
        if(err) {
            res.send('Failed to get data');
            console.log('Getting all from userInfo failed: ', err);
